...
public class Guppy {
    ...
    /**
     * Random probability helper, 0.5.
     */

    public static final double RANDOM_FIFTY_PERCENT_PROBABILITY = 0.5;
    /**
     * Random max number probability helper, 100.
     */
    public static final int RANDOM_MAX_NUMBER = 100;

    /**
     * Minimum weeks of female Guppy to be pregnant, 8.
     */
    public static final int MINIMUM_WEEK_BOUND_FOR_FEMALE_GUPPY = 8;
    /**
    ...
    /**
     * Spawns new fry from from Guppies with specific conditions.
     * Guppies must be female and 8 weeks old to spawn fry.
     * Fry have same genus and species from mother with 50% being female.
     * Generation number and health coefficient is passed on from mother with modified value.
     *
     * @return fry, ArrayList of Guppy
     */
    public ArrayList<Guppy> spawn() {
        ArrayList<Guppy> babyGuppies = new ArrayList<>();
        if (!this.getIsFemale() || this.getAgeInWeeks() < MINIMUM_WEEK_BOUND_FOR_FEMALE_GUPPY) {
            return null;
        } else {
            Random random = new Random();
            double spawnProbability = random.nextDouble();
            if (spawnProbability <= RANDOM_FIFTY_PERCENT_PROBABILITY) {
                int numberOfFry = random.nextInt(RANDOM_MAX_NUMBER);
